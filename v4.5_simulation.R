...
GetTopEigenvalue <- function(x, indices) {
  ...
  data <- x[indices, ]
  lambda1.hat <- svds(data, k = 1)$d / nrow(data) # rARPACK 
...
for (c in c(0, 3, 11, 50, 100)) {
  for (p in c(3, 30, 100, 150)) {
    simu.norm <- rbind(simu.norm,
                       PerfSimulation(N = 100, B = 100, n = 300,
                                      p = p, c = c, d.distr = 1))
  }
}
...
p.norm.bias <- ggplot(norm.data.plot, aes(x = r, y = bias.scaled)) +
  ...
  coord_cartesian(ylim = bias.scaled.lim1*10) +
  labs(title = "Z ~ Norm")
p.norm.bias
...
p.norm.var <- ggplot(norm.data.plot, aes(x = r, y = variance.ratio)) +
  ...
  scale_color_discrete(labels = labels) +
  scale_fill_discrete(labels = labels) +
  coord_cartesian(ylim = variance.ratio.lim1*7) +
  labs(title = "Z ~ Norm")
p.norm.var
...
p.ellip.norm.var <- ggplot(ellip.norm.data.plot, aes(x = r, y = variance.ratio)) +
  ...
  scale_color_discrete(labels = labels) +
  scale_fill_discrete(labels = labels) +
  coord_cartesian(ylim = variance.ratio.lim2*3) +
  labs(title = "Z ~ Ellip. Norm")
p.ellip.norm.var
