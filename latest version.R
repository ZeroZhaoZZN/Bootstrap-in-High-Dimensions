...
GetMatrix <- function(n, p, c, d.distr = 1) {
  ...
  lambda.sqrt <- diag(c(sqrt(lambda1), rep(1, p-1)))
  x <- d %*% z0 %*% v %*% lambda.sqrt
...
GetTopEigenvalue <- function(x, indices) {
  ...
  data <- x[indices, ]
  lambda1.hat <- (svds(data, k = 1)$d)^2 / nrow(data) # rARPACK 
...
PerfSimulation <- function(N, B, n, p, c, d.distr = 1) {
  ...
  for (i in 1:N) {
    ...
    std.err.boot[i] <- sqrt(
      sum((lambda1.hat.boot - mean(lambda1.hat.boot))^2) / (B - 1)
    )
  }
...
for (c in c(0, 3, 11, 50, 100)) {
  for (p in c(3, 30, 100, 150)) {
    simu.norm <- rbind(simu.norm,
                       PerfSimulation(N = 500, B = 200, n = 300,
                                      p = p, c = c, d.distr = 1))
  }
}
...
save.image("data_latest_version.RData")
...
norm.data.plot <- simu.norm %>%
  ...
  mutate(true.bias.scaled = abs(true.bias)/true.lambda1) %>%
...
p.norm.bias <- ggplot(norm.data.plot, aes(x = r, y = bias.scaled)) +
  ...
  geom_point(aes(x = r, y = true.bias.scaled, color = c),
             position = position_dodge(0.75),
             shape = 8) +
...
p.norm.var <- ggplot(norm.data.plot, aes(x = r, y = variance.ratio)) +
  ...
  scale_color_discrete(labels = labels) +
  scale_fill_discrete(labels = labels) +
  scale_y_continuous(breaks = c(0, 1, 4, 9, 16, 25), trans = "sqrt") +
  labs(title = "Z ~ Norm")
p.norm.var
...
p.ellip.unif.var <- ggplot(ellip.unif.data.plot, aes(x = r, y = variance.ratio)) +
  ...
  scale_color_discrete(labels = labels) +
  scale_fill_discrete(labels = labels) +
  scale_y_continuous(breaks = c(0, 1, 4, 9, 16, 25, 36), trans = "sqrt") +
  labs(title = "Z ~ Ellip. Uniform")
p.ellip.unif.var
...
p.ellip.exp.var <- ggplot(ellip.exp.data.plot, aes(x = r, y = variance.ratio)) +
  ...
  scale_y_continuous(breaks = c(0, 1, 4, 9), trans = "sqrt") +
  labs(title = "Z ~ Ellip. Exp")
p.ellip.exp.var
...
tab.norm.bias <- simu.norm %>%
  group_by(r, c) %>%
  summarise(
    median.bias = median(abs(bias.boot))
  )

tab.norm.var <- simu.norm %>%
  group_by(r, c) %>%
  summarise(
    median.variance = median(std.err.boot)
  )

# Z ~ Ellip. Norm

tab.ellip.norm.bias <- simu.ellip.norm %>%
  group_by(r, c) %>%
  summarise(
    median.bias = median(abs(bias.boot))
  )

tab.ellip.norm.var <- simu.ellip.norm %>%
  group_by(r, c) %>%
  summarise(
    median.variance = median(std.err.boot)
  )
...
tab.ellip.unif.bias <- simu.ellip.unif %>%
  group_by(r, c) %>%
  summarise(
    median.bias = median(abs(bias.boot))
  )

tab.ellip.unif.var <- simu.ellip.unif %>%
  group_by(r, c) %>%
  summarise(
    median.variance = median(std.err.boot)
  )
...
tab.ellip.exp.bias <- simu.ellip.exp %>%
  group_by(r, c) %>%
  summarise(
    median.bias = median(abs(bias.boot))
  )

tab.ellip.exp.var <- simu.ellip.exp %>%
  group_by(r, c) %>%
  summarise(
    median.variance = median(std.err.boot)
  )
